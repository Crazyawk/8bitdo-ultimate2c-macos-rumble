...
#     if (b[0] | b[1] | b[2] | b[3])
#         firmware_motor_schedule(255, 255, 30);
#     else
#         firmware_motor_schedule(0, 0, 1);
#     return 0;
# }
#
# firmware_motor_schedule is the existing routine at 0xfa68 that stores the two
# motor strengths and a countdown used by the firmware's own motor tick code.
HANDLER = bytes.fromhex(
    "10 65"
    "41 4a 84 4a c3 4a 02 4b"
    "23 03 0b 03 1a 03"
    "03 c0"
    "ff a0 ff a1 1e a2"
    "02 80"
...
def patch_firmware(src: Path, dst: Path) -> None:
    ...
    _att_num, perm, uuid_len, attr_len, uuid_ptr, value_ptr, write_cb, read_cb = attr
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Wire the Ultimate 2C BLE output report to the firmware motor scheduler."
    )
    parser.add_argument(
        "src",
        nargs="?",
    ...
    parser.add_argument(
        "dst",
        nargs="?",
        default="work/firmware-research/ultimate2c-controller-v1.09-bt-rumble-callback.dat",
    )
